...
class Order extends React.Component {
  ...
  updateHookah = (choice, name, price) => {
    ...
    const otherPrice =
      choice === 'bowl' ? hookah.filler.price : hookah.bowl.price

    const discount = this.state.discount
      ? (otherPrice + price) * (this.state.discount / 100)
      : 0
  ...
  onSubmitForm = (name, phone) => {
    ...
    const hookahStr = `<br>Чаша: <b>${
      hookah.bowl.name
    }</b><br>Наполнитель: <b>${hookah.filler.name}</b><br>Цена: <b>${
      hookah.priceHookah
    } руб.</b><br>`
    ...
    if (bowls.length === 0) {
      ...
      bowlsStr = bowls.reduce((acc, bowl, idx) => {
        if (idx === 0) {
          acc += `<br>Чаша: <b>${bowl.name}</b><br>Цена: <b>${
            bowl.price
          } руб.</b><br>`
        } else {
          acc += `____<br>Чаша: <b>${bowl.name}</b><br>Цена: <b>${
            bowl.price
          } руб.</b><br>`
        }
  ...
  render() {
    ...
    const { hookah, totalPrice, bowls, maxQuantity, discount } = this.state
          ...
          <div className="order__choice" id="bowl">
            <ul className="order__list">
              ...
              <ItemBowl
                ...
                clickItem={updateBowl}
              />
              <ItemBowl name={'Ананас'} price={2200} clickItem={updateBowl} />
            </ul>
          </div>
          ...
          <div className="modal__wrap">
            <div className="modal__header">
              ...
              <p className="modal__subtitle">
                Введите промокод, цена будет указана уже со скидкой
              </p>
            </div>
            <Promocode
